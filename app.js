...
// СИНХРОНИЗАЦИЯ ВРЕМЕНИ С СЕРВЕРОМ GOOGLE
...
let isDataLoaded = false; // ЖЕЛЕЗНЫЙ БЛОК ОТ ПЕРЕЗАПИСИ
...
dbRef.on('value', snap => {
    if (snap.exists() && snap.val()) {
        let data = snap.val(); 
        // ЗАЩИТА МАССИВОВ ОТ ПУСТОТЫ
        cloudState.tables = toArr(data.tables).map(t => ({...t, res: toArr(t.res), bar: toArr(t.bar)}));
        cloudState.checks = toArr(data.checks).map(c => ({...c, bar: toArr(c.bar), sessions: toArr(c.sessions)}));
...
function saveToCloud() { 
    if (!isDataLoaded) return; // НИКАКИХ ПЕРЕЗАПИСЕЙ ПУСТОТОЙ
...
function getActiveAdminName() {
    ...
    return 'Султан'; // Если никого не нашли
}
...
window.saveExpense = function() {
    ...
    let fullDesc = `[${cat}] ${desc}`; cloudState.expenses = toArr(cloudState.expenses); cloudState.expenses.push({ id: getNow(), sum: sum, desc: fullDesc, admin: activeAdmin, date: new Date(getNow()).toLocaleString() }); document.getElementById('expense-modal').style.display='none'; saveToCloud(); ui.alert("Расход записан на смену " + activeAdmin);
}
...
window.openStopTableModal = function(id) { 
    ...
    toArr(cloudState.checks).forEach(c => { options += `<option value="${c.id}">${c.name} (${c.details})</option>`; }); select.innerHTML = options; document.getElementById('stop-table-modal').style.display = 'flex';
}
...
window.moveTable = function(fromId) {
    ui.prompt('Пересадка', [{label: 'Номер нового стола (1-6)', type: 'number'}], (vals) => {
        ...
        tTo.active = true; tTo.start = tFrom.start; tTo.bar = toArr(tFrom.bar); tTo.paused = tFrom.paused; tTo.accCost = tFrom.accCost; tTo.accTime = tFrom.accTime; tTo.isTournament = tFrom.isTournament; tFrom.active = false; tFrom.start = null; tFrom.bar = []; tFrom.paused = false; tFrom.accCost = 0; tFrom.accTime = 0; tFrom.isTournament = false; saveToCloud();
    });
}
...
window.renderStockTab = function() {
    let invArr = toArr(cloudState.inventory); invArr.sort((a, b) => a.name.localeCompare(b.name)); if(currentStockCategory !== 'Все') { invArr = invArr.filter(i => (i.category || 'Прочее') === currentStockCategory); } let searchQ = document.getElementById('global-stock-search').value.toLowerCase(); if(searchQ) { invArr = invArr.filter(i => i.name.toLowerCase().includes(searchQ)); }
...
window.renderBarSearch = function() {
    let invArr = toArr(cloudState.inventory).filter(i => (i.qty||0) > 0); if(currentBarCategory !== 'Все') { invArr = invArr.filter(i => (i.category || 'Прочее') === currentBarCategory); }
...
// ИСПРАВЛЕНИЕ: ДЕТАЛИЗАЦИЯ ЧЕКОВ (СЕАНСЫ) И УЧЕТ ЗАРПЛАТЫ
function createOrMergeCheck(name, tableId, timeCost, barItems) {
    ...
    let sessionStr = tableId === "Бар" ? `Докупка на баре (${timeStr})` : `Стол ${tableId} (${startStr} - ${timeStr}): ${timeCost} ₸`;
    ...
    if(exist) { 
        ...
        let activeAdmin = localAuth.user.role === 'owner' ? getActiveAdminName() : localAuth.user.name;
        let newCheck = { id: getNow(), name: name, table: tableId, date: now.toLocaleDateString(), startTime: startStr, endTime: timeStr, duration: duration, timeCost: timeCost, barCost: barTotal, bar: bArr, total: timeCost + barTotal, discount: 0, sessions: [sessionStr], details: `Стол ${tableId}`, admin: activeAdmin }; 
...
window.openEditCheckModal = function(idx) { 
    editingCheckIdx = idx; let c = toArr(cloudState.checks)[idx]; document.getElementById('edit-check-name').value = c.name; document.getElementById('edit-check-time').value = c.timeCost; renderEditCheckBarItems(); 
    ...
    document.getElementById('edit-check-modal').style.display = 'flex'; 
}
...
window.addForgottenTableSession = function() {
    ui.prompt('Добавить забытый стол', [{label: 'Номер стола', type:'number'},{label: 'Начало (например 14:00)'},{label: 'Конец (например 15:30)'},{label: 'Сумма (₸)', type:'number'}], (vals) => {
        ...
        c.sessions.push(`Забытый Стол ${table} (${start} - ${end}): ${cost} ₸`);
        let currentInput = document.getElementById('edit-check-time'); currentInput.value = (parseInt(currentInput.value)||0) + cost;
        ui.alert("Стол добавлен! Нажмите СОХРАНИТЬ ИЗМЕНЕНИЯ.");
        window.openEditCheckModal(editingCheckIdx); // Ререндер
    });
}
window.removeSession = function(idx) {
    ui.confirm("Удалить этот сеанс?", () => { let c = cloudState.checks[editingCheckIdx]; c.sessions = toArr(c.sessions); c.sessions.splice(idx, 1); saveToCloud(); window.openEditCheckModal(editingCheckIdx); });
}
...
window.payPartialDebt = function() {
    ...
    ui.prompt('Частично в долг', [{label: `Сумма ОПЛАТЫ (из ${t} ₸)`, type: 'number'}, {label: 'Метод оплаты (Нал / QR)', value: 'Нал'}], (vals) => {
        ...
        let method = vals[1] || 'Наличные'; let debtAmount = t - paid; let c = cloudState.checks[currentCheckIndex];
        let activeAdmin = localAuth.user.role === 'owner' ? getActiveAdminName() : localAuth.user.name;
        
        cloudState.debts = toArr(cloudState.debts); let d = cloudState.debts.find(x => x.name.toLowerCase() === c.name.toLowerCase());
        let histStr = `+${debtAmount}₸ (Остаток с чека, Админ: ${activeAdmin})`;
...
window.openFullCheckObj = function(c) {
    try {
        ...
        let sessHtml = toArr(c.sessions).map(s => `<div>${s}</div>`).join('');
        document.getElementById('bill-sessions-container').innerHTML = sessHtml || 'Нет деталей';
...
function render() {
    ...
    document.getElementById('active-checks').innerHTML = toArr(cloudState.checks).map((c, i) => { 
        ...
        let sessHtml = toArr(c.sessions).map(s => `<div>${s}</div>`).join('');
        let timeInfo = `<div style="font-size:12px;color:var(--gray); font-weight:600; margin-top:5px;">${sessHtml || (c.startTime ? `🕒 ${c.startTime} - ${c.endTime} (${c.duration})` : '')}</div>`;
