...
💰 Выручка (Столы + Бар): ${state.totalRevenue} ₸
💵 Зарплата: ${salary} ₸
--------------------------
...
function stopTable(id) {
    ...
    if (confirm(`ИТОГО: ${total} ₸\nВремя: ${timeCost} ₸\nБар: ${barTotal} ₸\nОплачено?`)) {
        ...
        state.tableRevenue += timeCost;
        state.barRevenue += barTotal;
    } else {
...
function render() {
    ...
    state.tables.forEach(table => {
        ...
        card.innerHTML = `<div class="table-num">Стол ${table.id}</div><div style="font-size:10px; color:#888;">${table.active ? `👤 ${table.clientName}` : 'Свободен'}</div><div class="timer">${table.active ? new Date(Date.now() - table.startTime).toISOString().substr(11, 8) : '00:00:00'}</div><div class="cost">${money + barSum} ₸</div><button class="${table.active ? 'btn-stop' : 'btn-start'}" onclick="${table.active ? `stopTable(${table.id})` : `startTable(${table.id})`}">${table.active ? 'ОПЛАТА' : 'ПУСК'}</button><button class="btn-bar" onclick="addToBar(${table.id})" ${!table.active ? 'disabled' : ''}>+ БАР</button>`;
