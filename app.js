...
window.onload = () => { 
    ...
    // БЕЗОПАСНЫЙ СИГНАЛ ОНЛАЙН (Не перезаписывает базу, только обновляет время админа)
    setInterval(() => { 
...
function createOrMergeCheck(name, tableId, timeCost, barItems) {
    ...
    saveToCloud(); // <-- ВАЖНОЕ ИСПРАВЛЕНИЕ: МГНОВЕННОЕ СОХРАНЕНИЕ
}
