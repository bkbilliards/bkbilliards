...
function payDebt(idx) { 
    ...
    // Высчитываем 8% оригинальному админу
    ...
    // ДОБАВЛЯЕМ В АРХИВ КАК "ВОЗВРАТ ДОЛГА", ЧТОБЫ УШЛО В БУХГАЛТЕРИЮ
    if (method === 'Наличные') { localAuth.shiftCash = (localAuth.shiftCash || 0) + sum; }
    ...
    cloudState.archive.push({
        ...
        isDebtPayment: true // Флаг: не давать 8% текущему админу
    });
...
window.deductDebtFromAdmin = function(idx) {
    ...
    if(confirm(`Удержать долг (${d.total} ₸) из ЗП администратора ${d.admin || 'Неизвестно'}?`)) {
        ...
        d.total = 0;
        d.history.push(`УДЕРЖАНО С АДМИНА: ${d.admin}`);
        saveToCloud();
        alert("Долг успешно удержан из ЗП!");
    }
}

function delDebt(idx) { if(confirm("Хозяин, удалить этот долг навсегда?")) { cloudState.debts.splice(idx,1); saveToCloud(); } }
...
function renderTableBill() {
    if (!currentBillTableId) return; let t = cloudState.tables.find(x => x.id === currentBillTableId); if (!t) return;
    ...
    let barSum = 0; let html = (t.bar || []).map((b, i) => { barSum += b.price; return `<div class="edit-bar-item"><span>${b.name} (${b.price} ₸)</span> <button onclick="removeTableBarItem(${i})" class="btn-outline" style="color:var(--red); border-color:var(--red); padding:3px 8px; font-size:10px;">❌</button></div>`; }).join('');
...
function removeTableBarItem(idx) {
    if(!confirm("Убрать товар? Он вернется на склад.")) return; let t = cloudState.tables.find(x => x.id === currentBillTableId); let item = t.bar.splice(idx, 1)[0];
    let invItem = cloudState.inventory.find(x => x.name === item.name); if(invItem) invItem.qty += 1; saveToCloud(); renderTableBill(); 
}
...
function renderTables() {
    ...
    document.getElementById('tables-grid').innerHTML = cloudState.tables.map(t => {
        ...
        if(t.active && t.bar && t.bar.length > 0) {
            let grouped = {}; t.bar.forEach(i => { grouped[i.name] = grouped[i.name] || {q:0, p:i.price}; grouped[i.name].q++; });
...
function render() {
    ...
    // --- ОТОБРАЖЕНИЕ ДОЛГОВ С 15 ДНЯМИ ---
    document.getElementById('debts-list').innerHTML = (cloudState.debts||[]).map((d, i) => {
        let daysOld = 0;
        ...
        return `<tr>
            ...
            <td><span style="font-size:11px; color:var(--gray);">${(d.history||[]).join('<br>')}</span></td>
