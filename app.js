...
function login() {
    ...
    if (user && user.pin === pin) {
        localAuth = { isAuth: true, user: user, shiftStart: new Date().toLocaleString(), tableRev: 0, barRev: 0 };
...
function logout() {
    if (confirm("Вы точно хотите закрыть смену?")) {
        ...
        cloudState.history.push({ admin: localAuth.user.name, start: localAuth.shiftStart, end: new Date().toLocaleString(), timestamp: Date.now(), barRev: localAuth.barRev, tableRev: localAuth.tableRev, total: totalRev, sal: salary });
...
function resetDatabase() {
    if(confirm("ВНИМАНИЕ!\nВы уверены, что хотите полностью ОБНОВИТЬ (ОЧИСТИТЬ) БАЗУ?\n\nВсе чеки, долги, склады, истории смен и ЗАРПЛАТЫ АДМИНОВ удалятся навсегда!")) {
        ...
        saveToCloud(); localAuth = { isAuth: false, user: null, shiftStart: null, tableRev: 0, barRev: 0 }; saveLocalAuth();
...
function addCustomAdmin() {
    let name = prompt("Введите имя нового администратора:"); if(!name) return;
    let pin = prompt(`Придумайте PIN-код для ${name}:`);
    if(name && pin) { if(!cloudState.customAdmins) cloudState.customAdmins = []; cloudState.customAdmins.push({name: name, pin: pin, role: "admin"}); saveToCloud(); alert(`Администратор ${name} добавлен!`); }
}
...
function createOrMergeCheck(name, tableId, timeCost, barItems) {
    ...
    if(exist && confirm(`Объединить с чеком гостя "${exist.name}"?`)) { 
        ...
        let baseTotal = exist.timeCost + exist.barCost;
        exist.total = exist.discount ? Math.round(baseTotal * (1 - exist.discount/100)) : baseTotal;
        if(tableId !== "Бар") exist.details += ` + Стол ${tableId}`; 
    } 
    else { cloudState.checks.push({ id: Date.now(), name: name, table: tableId, date: new Date().toLocaleDateString(), timeCost: timeCost, barCost: barTotal, bar: barItems, total: timeCost + barTotal, discount: 0, details: `Стол ${tableId}` }); }
}

// === РЕДАКТИРОВАНИЕ И УДАЛЕНИЕ ЧЕКА ===
let editingCheckIdx = null;
function openEditCheckModal(idx) {
    editingCheckIdx = idx; let c = cloudState.checks[idx];
    document.getElementById('edit-check-name').value = c.name; document.getElementById('edit-check-time').value = c.timeCost;
...
function renderEditCheckBarItems() {
    ...
    if(html === '') html = '<span style="color:var(--gray); font-size:12px;">Пусто</span>';
    document.getElementById('edit-check-bar-list').innerHTML = html;
}
function removeBarItemFromCheck(itemIdx) {
    ...
    let baseTotal = c.timeCost + c.barCost;
    c.total = c.discount ? Math.round(baseTotal * (1 - c.discount/100)) : baseTotal;
    let invItem = cloudState.inventory.find(x => x.name === item.name);
...
function openPayModal(idx) { 
    ...
    if(c.discount && c.discount > 0) {
        document.getElementById('pay-total').innerHTML = `<span style="text-decoration:line-through; font-size:20px; color:var(--gray);">${origTotal} ₸</span><br>${c.total} ₸`;
        document.getElementById('pay-info').innerText = `${c.name} | ${c.details} (Скидка ${c.discount}%)`;
    } else {
...
function processPayment(method) {
    ...
    // Пересчет долей времени и бара с учетом скидки (чтобы сошлась бухгалтерия)
    let finalTimeCost = c.timeCost;
    let finalBarCost = c.barCost;
    if(c.discount && c.discount > 0) {
        let ratio = 1 - (c.discount / 100);
        finalTimeCost = Math.round(c.timeCost * ratio);
        finalBarCost = c.total - finalTimeCost; // Остаток накидываем на бар, чтобы сумма была ровной
        c.details += ` [Скидка ${c.discount}%]`;
    }
    ...
    localAuth.tableRev += finalTimeCost; 
    localAuth.barRev += finalBarCost; 
    
    c.timeCost = finalTimeCost; // Сохраняем в архив уже со скидкой
    c.barCost = finalBarCost;
    c.payMethod = method; 
...
function exportToExcel() {
    ...
    let csv = '\uFEFF'; csv += "АДМИН;НАЧАЛО;КОНЕЦ;БАР (ТНГ);СТОЛЫ (ТНГ);КАССА ОБЩАЯ (ТНГ);ВЫДАНО ЗП (ТНГ)\n";
    currentFilteredHistory.forEach(h => { csv += `${h.admin};${h.start};${h.end};${h.barRev||0};${h.tableRev||0};${h.total||0};${h.sal||0}\n`; });
...
function renderAccounting() {
    ...
    document.getElementById('history-list').innerHTML = currentFilteredHistory.map(h => `<tr><td><b>${h.admin}</b></td><td><span style="font-size:11px; color:var(--gray);">${h.start} - ${h.end}</span></td><td>${h.barRev} ₸</td><td>${h.tableRev} ₸</td><td class="gold-text"><b>${h.total} ₸</b></td><td><b style="color:var(--green);">${h.sal} ₸</b></td></tr>`).join('');
}
...
function render() {
    if (!localAuth.isAuth) { 
        ...
        if(currentAdminCount !== lastAdminCount) {
            ...
            document.getElementById('staff-select').innerHTML = html; lastAdminCount = currentAdminCount;
        }
    ...
    document.getElementById('active-checks').innerHTML = (cloudState.checks||[]).map((c, i) => { 
        ...
        return `<div class="check-row"><div style="flex:1;"><div><b style="font-size:20px; color:var(--gold);">${c.name}</b> <span style="font-size:11px;color:var(--gray);margin-left:10px;">${c.date}</span></div><div style="font-size:13px;color:var(--gray);margin-top:8px;">${c.details} (${c.timeCost} ₸) ${bHtml?`<br>🍸 Бар: ${bHtml} (${c.barCost} ₸)`:''}</div><div style="font-size:24px;font-weight:800;margin-top:10px;">${c.total} ₸ ${discountHtml}</div></div><div style="display:flex; flex-direction:column; gap:5px;"><button onclick="openPayModal(${i})" class="btn-gold shadow-gold" style="padding:15px; border-radius:12px;">ОПЛАТА</button><button onclick="openEditCheckModal(${i})" class="btn-outline" style="border-color:#444; color:#aaa; font-size:10px;">⚙️ РЕДАКТИРОВАТЬ</button><button onclick="deleteCheck(${i})" class="btn-outline" style="border-color:var(--red); color:var(--red); font-size:10px;">🗑️ УДАЛИТЬ ЧЕК</button></div></div>`; 
    }).join('');
    ...
    document.getElementById('stock-list').innerHTML = (cloudState.inventory||[]).map((i, idx) => `<tr><td><b style="color:var(--white);">${i.name}</b></td><td><b style="font-size:16px;">${i.qty} шт</b></td><td class="gold-text"><b style="font-size:16px;">${i.price} ₸</b></td><td style="display:flex; gap:5px; flex-wrap:wrap;"><button onclick="editItemQty(${idx})" class="btn-outline" style="padding:6px 10px; font-size:10px;">✏️ КОЛ-ВО</button><button onclick="renameItem(${idx})" class="btn-outline" style="padding:6px 10px; font-size:10px;">✏️ ИМЯ</button><button onclick="editItemPrice(${idx})" class="btn-outline" style="padding:6px 10px; font-size:10px;">✏️ ЦЕНА</button><button onclick="delItem(${idx})" class="btn-red" style="padding:6px 10px; font-size:10px; width:auto; margin-top:0;">❌</button></td></tr>`).join('');
