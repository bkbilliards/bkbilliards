...
function processPayment(method) {
    ...
    localAuth.tableRev += Math.round(c.timeCost * discRatio);
    localAuth.barRev += (c.total - Math.round(c.timeCost * discRatio));
...
function render() {
    ...
    if(isOwner) {
        ...
        document.getElementById('custom-admins-list').innerHTML = (cloudState.customAdmins || []).map(a => `<div class="dash-card">${a.name} (PIN: ${a.pin})</div>`).join('');
    }
}
...
function renderAccounting() {
    ...
    document.getElementById('history-list').innerHTML = (cloudState.history || []).map(h => `<tr><td>${h.admin}</td><td>${h.end}</td><td>${h.total} (${h.diff})</td><td>${h.sal}</td></tr>`).join('');
}

setInterval(() => { if(localAuth.isAuth) render(); }, 1000);

function deleteCheck(i) { if(confirm("УДАЛИТЬ ЧЕК?")) { cloudState.checks.splice(i,1); saveToCloud(); } }
function resetDatabase() { if(confirm("СБРОСИТЬ ВСЕ ДАННЫЕ?")) { cloudState = { tables: Array.from({length: 6}, (_, i) => ({ id: i + 1, active: false })), checks: [], inventory: [], history: [], ownerAcc: {}, customAdmins: [] }; saveToCloud(); location.reload(); } }
