...
// ИСПРАВЛЕНИЕ: ВЫДАЧА АВАНСА МЕНЯЕТ ДОЛГ К ВЫПЛАТЕ
window.giveAdvance = function(name, debt) {
    ui.prompt('Аванс', [{label:`Аванс для ${name} (Долг: ${debt}₸)`, type:'number'}], (v) => {
        let adv = parseInt(v[0]);
        if (!isNaN(adv) && adv > 0) {
            cloudState.ownerAcc[name] = debt - adv;
            saveToCloud(); render();
        }
    });
}

// ИСПРАВЛЕНИЕ: ИЗМЕНЕНИЕ ЦИФРЫ ДОЛГА
window.changeBalance = function(name, debt) {
    ui.prompt('Изменить баланс', [{label:'Новый баланс', type:'number', value:debt}], (v) => {
        let newBal = parseInt(v[0]);
        if (!isNaN(newBal)) {
            cloudState.ownerAcc[name] = newBal;
            saveToCloud(); render();
        }
    });
}

// ИСПРАВЛЕНИЕ: ШТРАФ АДМИНУ
window.issuePenalty = function(name, debt) {
    ui.prompt('Штраф', [{label:'Сумма штрафа', type:'number'}], (v) => {
        let pen = parseInt(v[0]);
        if (!isNaN(pen) && pen > 0) {
            cloudState.ownerAcc[name] = debt - pen;
            saveToCloud(); render();
        }
    });
}

// ИСПРАВЛЕНИЕ: ПОЛНЫЙ РАСЧЕТ
window.fullPayoff = function(name) {
    ui.confirm(`Выдать полный расчет администратору ${name}?`, () => {
        cloudState.ownerAcc[name] = 0;
        saveToCloud(); render();
    });
}
